...
# New mode (not working on newer Cinnamon versions anymore)
#RATE_NEW=-r
...
function rotate {
    ...
    xrandr --output $DNAME --mode $MODE --rotate $ROT_NEW # Rotate screen
    xrandr --output $DNAME --gamma $GAMMA --brightness $BRIGHT # Final fixes
...
# ROTATE_ACTIVE=$(gsettings get org.cinnamon.settings-daemon.peripherals.touchscreen orientation-lock) #(not working on newer Cinnamon versions anymore)
ROTATE_ACTIVE=false

    if ! $ROTATE_ACTIVE; then # Cinnamon display auto-rotate setting
